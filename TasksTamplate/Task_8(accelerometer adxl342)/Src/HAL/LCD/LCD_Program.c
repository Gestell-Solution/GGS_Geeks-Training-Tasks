...
/**
 * @brief Generates one pulse on the Enable pin so the LCD latches the data lines.
 */
...
/**
 * @brief Sends one byte to the LCD, as data or as an instruction.
 * @details In 4-bit mode the upper nibble is sent first, then the lower one.
 * @param[in] Byte           Byte to send.
 * @param[in] RegisterSelect ::High for data, ::Low for an instruction.
 * @param[in] ModeType       ::Lcd_4bitMode or ::Lcd_8bitMode.
 */
