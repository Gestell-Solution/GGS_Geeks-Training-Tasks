...
static uint16_t UART_ReadDataRegister(void)
{
    ...
    {
        if (READ_BIT(UCSRB_Reg, Uart_RXB8) == 1U)
        {
...
void UART_Init(Uart_Config_t Uart_Config)
{
    ...
    {
        CLEAR_BIT(&UCSRA_Reg, Uart_U2X);
    }
    else if (Uart_Config.SpeedMode == Uart_DoubleSpeed)
    {
        SET_BIT(&UCSRA_Reg, Uart_U2X);
    }
    else
    {
...
void UART_SendBytePolling(uint16_t Data)
{
    ...
    if (GlobalUart_Config.SizeCharacterSelect == Uart_9BitSize)
    {
        CLEAR_BIT(&UCSRB_Reg, Uart_TXB8);
        if (((Data >> 8U) & 0x01U) == 1U)
        {
            SET_BIT(&UCSRB_Reg, Uart_TXB8);
        }
    }
    else
    {
 ...
 * These wait for at most Timeout_ms milliseconds instead of forever, so a
 * silent/disconnected peer (a Bluetooth module with no power, out of
 * range, etc.) can never hang the MCU indefinitely - it always gets
 * control back within a known, bounded time.
 * NOTE: this still stalls the caller for up to Timeout_ms (it is a
 * *bounded*-blocking call, built on _delay_ms()), not a fully cooperative
 * / non-blocking design. That is a deliberate, documented scope choice:
 * it matches the timeout style already used in this codebase and needs no
 * extra timer/ISR infrastructure. A future iteration could replace the
 * _delay_ms() tick with a free-running timer/ISR tick counter to make
 * this genuinely non-blocking for a cooperative superloop.
 * ================================================================== */
...
{
    SET_BIT(&UCSRB_Reg, Uart_RXEN);
}
void UART_EnableTX(void)
{
    SET_BIT(&UCSRB_Reg, Uart_TXEN);
}
void UART_DisableRX(void)
{
    CLEAR_BIT(&UCSRB_Reg, Uart_RXEN);
}
void UART_DisableTX(void)
{
    CLEAR_BIT(&UCSRB_Reg, Uart_TXEN);
}
...
{
    SET_BIT(&UCSRB_Reg, Uart_RXCIE);
}
void UART_EnableTXInterrupt(void)
{
    SET_BIT(&UCSRB_Reg, Uart_TXCIE);
}
void UART_EnableREInterrupt(void)
{
    SET_BIT(&UCSRB_Reg, Uart_UDRIE);
}
void UART_DisableRXInterrupt(void)
{
    CLEAR_BIT(&UCSRB_Reg, Uart_RXCIE);
}
void UART_DisableTXInterrupt(void)
{
    CLEAR_BIT(&UCSRB_Reg, Uart_TXCIE);
}
void UART_DisableREInterrupt(void)
{
    CLEAR_BIT(&UCSRB_Reg, Uart_UDRIE);
}
