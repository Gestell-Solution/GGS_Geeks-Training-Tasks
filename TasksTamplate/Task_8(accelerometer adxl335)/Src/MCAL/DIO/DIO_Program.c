 ...
 * @details Implements the pin and port level digital I/O functions declared
 *          in DIO_Interface.h. The public API is documented in the header.
 * @version 1.1.0
...
void DIO_InitPin(uint8_t GroupName, uint8_t PinNumber, uint8_t DirState)
{
    if ((GroupName <= Dio_GroupD) && (PinNumber <= Dio_Pin7))
    {
        ...
        {
            /* Invalid direction: request ignored. */
        }
    }
}
...
{
    if ((PinState != Null) && (GroupName <= Dio_GroupD) && (PinNumber <= Dio_Pin7))
    {
        switch (GroupName)
        {
            case Dio_GroupA:
