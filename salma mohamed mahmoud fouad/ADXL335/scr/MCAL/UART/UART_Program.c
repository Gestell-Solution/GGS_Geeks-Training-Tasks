...
{     
    ...
    // 2- Check on size 
    if(GlobaleUart_Config.SizeCharacterSelect==Uart_9BitSize)//chaged from 9 to Uart_9BitSize
    {
        //TXB8 -> UCSRB
...
uint16_t UART_ReceiveBytePolling()
{
    ...
    if(GlobaleUart_Config.SizeCharacterSelect==Uart_9BitSize)
    {
        ...
        {
            SetBit(LocalData,Uart_RXB8);//or write 8 
        }
    }
    LocalData|=UDR_Reg;
    return LocalData;
...
void UART_SendStringPolling(uint8_t* String)
{
    ...
    if(String != Null)
    {
        ...
        {
            // Wait for UDRE flag before each byte
            while(!(UCSRA_Reg & (1 << Uart_UDRE)));
            UDR_Reg = String[LocalCount];
            LocalCount++;
            _delay_us(10);
        }
    }
